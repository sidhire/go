...
class raw_env(AECEnv):
    ...
    def _overwrite_go_global_variables(self, board_size: int):
        ...
        go_base.NEIGHBORS = {
            (x, y): list(
                filter(self._check_bounds, [(x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)])
            )
    ...
    def step(self, action):
        ...
        if self.go_game.is_game_over():
            ...
            self.rewards = self._convert_to_dict(self._encode_rewards(self.go_game.result()))
            self.next_legal_moves = [self._N * self._N]
        else:
            self.next_legal_moves = self._encode_legal_actions(self.go_game.all_legal_moves())
